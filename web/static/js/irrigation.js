...
async function setMode(mode){if(mode==='schedule'){showToast('스케줄 모드는 준비 중입니다.');return;}try{const res=await apiPost('/api/irrigation/mode',{mode});if(res.success){state.mode=mode;applyModeUI(mode);showToast(res.message||`모드 변경: ${mode}`);}else{showToast(res.error||'모드 변경 실패');}}catch(e){showToast('서버 오류: '+e.message);}}
